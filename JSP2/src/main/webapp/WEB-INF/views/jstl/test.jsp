...
<body>
	...
	<c:set var="num3" value="30" scope="session"></c:set>
	<c:set var="num4" value="40" scope="application"></c:set>
    ...
    * 주의사항 : else 구문이 없다!
	...
	<c:set var="test3" value="양념닭갈비"/>
	<c:if test='${test3 ==  "숯불닭갈비"}'>
		<h4>${test3} 맛있겠다</h4>
	</c:if>
	...
	<c:choose>
		<%--
			...
			1) null인 경우 true
			2) 연산 대상이 배열/컬렉션인데
			   내부에 아무 요소 없는 경우 true
			
			나머지는 false
		 --%>
		
		<c:when test="${empty param.age }">
			<h4 style="color:red">age값이 없습니다</h4>
		</c:when>
